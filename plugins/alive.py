...
@Andencento.on(events.callbackquery.CallbackQuery(data=re.compile(b"demploy")))
...
@Andencento.on(events.callbackquery.CallbackQuery(data=re.compile(b"bamck")))
async def bamck(event):
    await event.edit(PM_IMG, caption=pm_caption, buttons=btn)



@Andencento.on(events.callbackquery.CallbackQuery(data=re.compile(b"stling")))
...
@Andencento.on(events.callbackquery.CallbackQuery(data=re.compile(b"bomk")))
async def bamck(event):
    await event.edit(PM_IMG, caption=pm_caption, buttons=btn)
