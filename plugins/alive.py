...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"demploy")))
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"bamck")))
async def bamck(event):
    await event.edit(PM_IMG, caption=pm_caption, buttons=btn)
    
    
    
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"stling")))
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"bomk")))
async def bamck(event):
    await event.edit(PM_IMG, caption=pm_caption, buttons=btn)
